...
function loadImage(file) {
  ...
  img.onload = () => {
    ...
    // Default: ignore app header and bottom buttons from phone screenshots.
    state.crop = guessCrop(canvas.width, canvas.height);
    draw(); render(); setStatus('Screenshot geladen. Bereich ggf. ums Spielfeld ziehen.');
  };
  img.src = URL.createObjectURL(file);
}

function guessCrop(w,h){
  return { x: Math.round(w*0.07), y: Math.round(h*0.20), w: Math.round(w*0.86), h: Math.round(h*0.58) };
}

cropBtn.addEventListener('click', () => {
  state.cropMode = !state.cropMode;
  cropBtn.textContent = state.cropMode ? 'Bereich aktiv' : 'Bereich wählen';
  setStatus(state.cropMode ? 'Rechteck um das Spielfeld ziehen' : 'Bereit');
  draw();
});

detectBtn.addEventListener('click', () => {
  if (!state.image) return setStatus('Erst Screenshot laden');
  detectIslands(); draw(); render();
});

addIslandBtn.addEventListener('click', () => {
  state.addMode = !state.addMode;
  addIslandBtn.textContent = state.addMode ? 'Tippen...' : '+ Insel';
  setStatus(state.addMode ? 'Auf das Board tippen, um Insel einzufügen' : 'Bereit');
});
...
applyMoveBtn.addEventListener('click', () => {
  ...
  state.pendingMove = null; applyMoveBtn.disabled = true;
  moveBox.textContent = 'Move angewendet.'; draw(); render();
});

clearBtn.addEventListener('click', () => {
  state.islands = []; state.bridges = []; state.selectedId = null; state.nextId = 1; state.pendingMove = null;
  draw(); render(); setStatus('Zurückgesetzt');
});
...
canvas.addEventListener('pointermove', e => {
  if (state.cropMode && state.drag) {
    state.drag.current = eventPoint(e);
    draw();
  }
});
...
canvas.addEventListener('pointerup', e => {
  ...
  if (state.cropMode && state.drag) {
    ...
    if (w > 40 && h > 40) state.crop = { x: Math.round(x), y: Math.round(y), w: Math.round(w), h: Math.round(h) };
    state.drag = null; draw(); setStatus('Erkennungsbereich gesetzt'); return;
  }
  handleTap(p, e);
});
...
function handleTap(p,e){
  ...
  if (e.shiftKey || state.addMode) {
    addIsland(p.x,p.y,1); state.addMode=false; addIslandBtn.textContent='+ Insel'; draw(); render(); return;
  }
  if (!hit) { state.selectedId=null; draw(); render(); return; }
  if (state.selectedId && state.selectedId !== hit.id) {
    const a = islandById(state.selectedId), b = hit;
    if (areAligned(a,b) && !wouldCross(a.id,b.id)) applyBridge(a.id,b.id,(bridgeCount(a.id,b.id)+1)%3);
...
function detectIslands(){
  ...
  const maxR = Math.min(32, Math.round(Math.min(crop.w,crop.h)*0.060));
  const candidates = [];
  const step = 4;
  for (let y = crop.y + maxR; y < crop.y + crop.h - maxR; y += step) {
    for (let x = crop.x + maxR; x < crop.x + crop.w - maxR; x += step) {
      ...
      for (let r = minR; r <= maxR; r += 2) {
        const s = circleScore(img, x, y, r);
        if (!best || s.score > best.score) best = {x,y,r,...s};
      }
      if (best && best.score > 0.58 && best.inner > 0.03) candidates.push(best);
    }
  }
  candidates.sort((a,b) => b.score - a.score);
  const picked = [];
  for (const c of candidates) {
    if (picked.some(p => Math.hypot(p.x-c.x,p.y-c.y) < Math.max(p.r,c.r)*1.4)) continue;
    picked.push(c);
  }
  // Snap near positions to a cleaner grid, but keep original if no grid is clear.
  ...
  setStatus(`${state.islands.length} Inseln im gewählten Bereich erkannt. Zahlen bitte korrigieren.`);
}
...
function isBright(data, w, x, y){
  ...
  return avg>155 && diff<85;
}
function circleScore(img,x,y,r){
  const data=img.data,w=img.width;
  let ring=0,total=0,inner=0,innerTotal=0;
  for(let a=0;a<Math.PI*2;a+=Math.PI/18){
    for(const rr of [r-1,r,r+1]){ total++; if(isBright(data,w,x+Math.cos(a)*rr,y+Math.sin(a)*rr)) ring++; }
  }
  for(let yy=-r*.45; yy<=r*.45; yy+=3){
    for(let xx=-r*.45; xx<=r*.45; xx+=3){ innerTotal++; if(isBright(data,w,x+xx,y+yy)) inner++; }
  }
...
function drawBaseImage(){
  ...
  else { ctx.fillStyle='#070b10'; ctx.fillRect(0,0,canvas.width,canvas.height); }
}
function draw(){
  drawBaseImage();
  drawCrop(); drawBridges(); if(state.pendingMove) drawSuggestedMove(state.pendingMove); drawIslands();
}
function drawCrop(){
  const c = state.drag ? rectFromDrag(state.drag) : state.crop;
  if(!c) return;
  ctx.save(); ctx.strokeStyle=state.cropMode?'#ffd36f':'rgba(255,211,111,.8)'; ctx.lineWidth=3; ctx.setLineDash([8,6]); ctx.strokeRect(c.x,c.y,c.w,c.h); ctx.restore();
}
...
function drawBridgeLine(a,b,count,color,width){
  ...
  const horizontal=Math.abs(a.x-b.x)>Math.abs(a.y-b.y); const offsets=count===2?[-5,5]:[0];
  for(const o of offsets){ ctx.beginPath(); ctx.moveTo(a.x+(horizontal?0:o),a.y+(horizontal?o:0)); ctx.lineTo(b.x+(horizontal?0:o),b.y+(horizontal?o:0)); ctx.stroke(); }
}
function drawIslands(){
  const r=islandRadius();
  for(const i of state.islands){
    ctx.beginPath(); ctx.arc(i.x,i.y,r,0,Math.PI*2); ctx.fillStyle=i.id===state.selectedId?'#15384a':'#111923'; ctx.fill();
    ctx.lineWidth=i.id===state.selectedId?5:4; ctx.strokeStyle='#eef3f8'; ctx.stroke();
    ctx.fillStyle='#eef3f8'; ctx.font=`800 ${Math.round(r*1.25)}px system-ui`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(i.value,i.x,i.y+1);
  }
}
...
function renderIslands(){
  ...
  for(const i of state.islands){
    ...
    const del=document.createElement('button'); del.textContent='×'; del.addEventListener('click',()=>removeIsland(i.id));
...
function renderMove(move){
  ...
  moveBox.innerHTML=`<b>${move.action}</b><br>#${a.id} ↔ #${b.id}: auf <b>${move.targetCount}</b> Brücke(n).<br><br>${move.reason}`;
}
function renderValidation(){
  validationBox.innerHTML = state.islands.map(i=>{ const u=usedCount(i.id), r=i.value-u; const cls=r<0?'bad':r===0?'ok':''; return `<div class="${cls}">#${i.id}: ${u}/${i.value}${r>0?`, Rest ${r}`:''}</div>`; }).join('') || '<div class="muted">Noch keine Inseln.</div>';
}
...
function wouldCross(aId,bId){
  const a=islandById(aId), b=islandById(bId); if(!a||!b) return true; const s=segment(a,b);
  for(const br of state.bridges){ if([br.a,br.b].includes(aId)||[br.a,br.b].includes(bId)) continue; const c=islandById(br.a), d=islandById(br.b); const t=segment(c,d); if(s.h===t.h) continue; const h=s.h?s:t, v=s.h?t:s; if(v.x>h.x1&&v.x<h.x2&&h.y>v.y1&&h.y<v.y2) return true; }
  return false;
}
function visibleNeighbors(id){
  const i=islandById(id); if(!i) return [];
  const dirs=[{p:j=>j.x>i.x&&Math.abs(j.y-i.y)<24,s:(a,b)=>a.x-b.x},{p:j=>j.x<i.x&&Math.abs(j.y-i.y)<24,s:(a,b)=>b.x-a.x},{p:j=>j.y>i.y&&Math.abs(j.x-i.x)<24,s:(a,b)=>a.y-b.y},{p:j=>j.y<i.y&&Math.abs(j.x-i.x)<24,s:(a,b)=>b.y-a.y}];
  return dirs.map(d=>state.islands.filter(j=>j.id!==id&&d.p(j)).sort(d.s)[0]).filter(Boolean).filter(n=>!wouldCross(id,n.id));
}
...
function findNextMove(){
  for(const i of state.islands){
    const rem=remaining(i.id); if(rem<=0) continue; const ns=possibleNeighbors(i.id); if(!ns.length) continue;
    const capSum=ns.reduce((s,n)=>s+capacity(i.id,n.id),0);
    if(rem===capSum){ const n=ns.find(n=>capacity(i.id,n.id)>0); return {a:i.id,b:n.id,targetCount:bridgeCount(i.id,n.id)+capacity(i.id,n.id),action:'Alle Kapazitäten nutzen',reason:`Restbedarf ${rem}, mögliche Gesamtkapazität ${capSum}. Also muss jede mögliche Verbindung maximal gesetzt werden.`}; }
    for(const n of ns){ const otherCap=capSum-capacity(i.id,n.id), min=rem-otherCap; if(min>0&&bridgeCount(i.id,n.id)<min) return {a:i.id,b:n.id,targetCount:min,action:'Mindestbrücke erzwungen',reason:`Andere Nachbarn können zusammen höchstens ${otherCap} liefern. Deshalb braucht diese Verbindung mindestens ${min}.`}; }
    if(rem===ns.length){ const n=ns.find(n=>bridgeCount(i.id,n.id)===0); if(n) return {a:i.id,b:n.id,targetCount:1,action:'Eine Brücke zu jedem Nachbarn',reason:`Restbedarf ${rem} und genau ${ns.length} mögliche Nachbarn.`}; }
  }
  return null;
}
